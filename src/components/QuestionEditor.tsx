...
export default function QuestionEditor({ question, onUpdate, onDelete, index }: QuestionEditorProps) {
  ...
  return (
    <Card className="p-4 sm:p-6 space-y-3 sm:space-y-4 hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 space-y-3 sm:space-y-4">
          <div className="flex items-center justify-between">
            <Label className="text-xs sm:text-sm font-semibold text-gray-700">Question {index + 1}</Label>
            <Button 
              variant="ghost" 
              size="sm" 
              onClick={onDelete}
          ...
          <div className="grid gap-3 sm:gap-4">
            <div>
              <Label htmlFor={`question-${question.id}`} className="text-xs sm:text-sm">Question</Label>
              <Input
            ...
            <div>
              <Label htmlFor={`type-${question.id}`} className="text-xs sm:text-sm">Type de question</Label>
              <Select
                value={question.type}
            ...
            {question.type === 'scale' && (
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <Label htmlFor={`scale-min-${question.id}`} className="text-xs sm:text-sm">Minimum</Label>
                  <Input
                ...
                <div>
                  <Label htmlFor={`scale-max-${question.id}`} className="text-xs sm:text-sm">Maximum</Label>
                  <Input
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                className="h-4 w-4"
              />
              <Label htmlFor={`required-${question.id}`} className="text-xs sm:text-sm font-normal cursor-pointer">
